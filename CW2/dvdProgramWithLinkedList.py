...
def return_customer_index_to_beginning():
    try:
        customer_list.return_to_the_beginning()
        #print("\n\nCustomer index returned to the starting point\n\n")
    except:
        pass
...
def print_all_customers():   # 8. Print a list of all the DVDs rented by each customer
    return_customer_index_to_beginning()
    print("\n\n Printing out all the customers\n\n")
    while customer_list.node is not None:
        customer_list.node.print_data()
        customer_list.node = customer_list.node.next
...
def rent_dvd():   # 1. Rent a DVD (check out)
    ...
    while customer_list.node != None: # keeps looping until we hit a dead end
        print("\n\ncurrent acc number:" + customer_list.node.value.account_number + "\n\n")
        if customer_list.node.value.account_number == account_number: # checks if the account numbers are the same
            ...
            while True:
                ...
                while dvd_list.node != None:
                    if dvd_list.node.value.movie_name == movie_name:
                        ...
                        if dvd_list.node.value.copies>=1:
                            ...
                            customer_list.node.value.add_rented_dvd(dvd_list.node.value)
                            print(dvd_list.node.value.movie_name+" has been rented")
                        else:
                            print("\n\nThere isn't any dvds to rent")
                        break
                    # If the current dvd isn't the one we're looking for, we'll move on to the next dvd
                    dvd_list.node = dvd_list.node.next
                ...
                if stop == "stop":
                    return
        # If the current customer account isn't the one we're looking for, we'll move on to the next customer account
        customer_list.node = customer_list.node.next
...
def return_dvd(): # 2. Return a DVD (check in)
    return_customer_index_to_beginning()
    account_number = input("Enter your account number: ")
    while customer_list.node != None:
        print("\n\ncurrent acc number:"+ customer_list.node.value.account_number+"\n\n")
        if customer_list.node.value.account_number == account_number:
            print("Customer account found")
            while True:
                ...
                while dvd_list.node != None:
                    if dvd_list.node.value.movie_name == movie_name:
                        print("DVD found")
                        if customer_list.node.value.rented_dvds.__contains__(dvd_list.node.value):
                            dvd_list.node.value.copies += 1
                            customer_list.node.value.remove_rented_dvd(dvd_list.node.value)
                            print(dvd_list.node.value.movie_name + " has been returned")
                        else:
                            print("The customer account "+customer_list.node.value.account_number+" has not rented the movie "+
...
def remove_customer():
    ...
    while customer_list.node != None:
        if customer_list.node.value.account_number == account_number:
            print("Customer account found")
            if customer_list.node.previous != None and customer_list.node.next != None:
                customer_list.node.previous.next = customer_list.node.next
            elif customer_list.node.previous == None:
                customer_list.start = customer_list.node.next
            elif customer_list.node.next == None:
                customer_list.node.previous.next = None
            print(account_number+" has been removed")
            return
        else:
            customer_list.node = customer_list.node.next
...
customer_list = customerList() # 7. Maintain a customer database
